    ...
    definitions: [
      {
        ...
        description: "Создать периодическую задачу. Вызывай когда пользователь просит делать что-то регулярно/периодически. Не переспрашивай — сразу создавай задачу на основе запроса.",
        parameters: {
          type: "object",
          properties: {
            name: { type: "string", description: "Короткое название задачи (до 50 символов), на языке пользователя" },
            cron_expression: {
              type: "string",
              description: [
                "Расписание выполнения. Два формата:",
                "1) Cron (5 полей, минимум 1 мин): */5 * * * * — каждые 5 минут",
                "2) Для секунд: @every <миллисекунды>. Примеры: @every 10000 = 10 сек, @every 20000 = 20 сек, @every 30000 = 30 сек, @every 60000 = 1 мин.",
                "ВАЖНО: НЕ используй 6-полный cron. Для интервалов < 1 мин используй ТОЛЬКО @every.",
              ].join(" "),
            },
            prompt: { type: "string", description: "Промпт для LLM при каждом выполнении. Пиши на том же языке, на котором общается пользователь. Промпт должен быть самодостаточным — без ссылок на контекст диалога." },
          },
...
`;
        ...
        if (schedulerToolDefs.length > 0) {
          ...
          toolHint += `У тебя есть инструменты планировщика. Правила:
- Используй их когда пользователь просит делать что-то периодически/регулярно/по расписанию.
- НЕ вызывай их для обычного разговора, вопросов, просьб не связанных с расписанием.
- Не переспрашивай параметры — сразу создавай задачу на основе запроса пользователя.
- Ты — универсальный ассистент, планировщик — лишь одна из твоих функций. На вопросы вроде "что ты умеешь?" отвечай про все свои возможности, не только про расписание.
