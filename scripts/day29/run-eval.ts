...
import { CONFIGS, DAY29_QUESTION_IDS, type Day29Config } from "./configs";
import { captureVramSnapshot, type VramSnapshot } from "./vram";
...
function validateConfigSupported(_config: Day29Config): void {
  // All features (promptVariant, numCtx, temperature override) supported in current impl.
}

async function main(): Promise<void> {
  const flags = parseArgs(Bun.argv.slice(2));
  const configName = flags.config ?? "c0";
  const config = CONFIGS[configName];
  if (!config) {
    fail(`Unknown config "${configName}". Available: ${Object.keys(CONFIGS).join(", ")}`);
  }
  validateConfigSupported(config);
  ...
  const cloudModel =
    process.env.DAY29_CLOUD_MODEL?.trim() ||
    modelRepo.getRole("chat")?.id ||
    "openai/gpt-5-nano";
  const cloudJudgeModel =
    process.env.DAY29_CLOUD_JUDGE_MODEL?.trim() ||
  ...
  const cloudMaxCompletionTokens = Number(process.env.DAY29_CLOUD_MAX_TOKENS?.trim() || "4000");
  ...
  const wanted = new Set(DAY29_QUESTION_IDS);
  const questions = allQuestions.filter((q) => wanted.has(q.id));
  if (questions.length !== DAY29_QUESTION_IDS.length) {
    fail(
      `Expected ${DAY29_QUESTION_IDS.length} questions (${DAY29_QUESTION_IDS.join(", ")}), got ${questions.length} from ${questionsPath}`,
    );
  }
  ...
  for (const q of questions) {
    if (q.outOfScope) {
      if (!q.topicalTerms || q.topicalTerms.length === 0) {
        fail(`OOS question ${q.id} missing topicalTerms in ${questionsPath}`);
      }
      oosIndex.set(q.id, q);
    }
  }

  console.log(pc.bold(`Day 29 — ${config.name}: ${config.description}`));
  console.log(pc.dim(`  model:          ${localModel}`));
  console.log(pc.dim(`  promptVariant:  ${config.promptVariant}`));
  console.log(pc.dim(`  temperature:    ${config.temperature}`));
  console.log(pc.dim(`  maxTokens:      ${config.maxCompletionTokens}`));
  console.log(pc.dim(`  numCtx:         ${config.numCtx ?? "(default)"}`));
  console.log(pc.dim(`  strategy:       ${strategy} / threshold=${threshold}`));
  console.log(pc.dim(`  questions:      ${questions.map((q) => q.id).join(", ")}`));
  console.log(pc.dim(`  runs per cell:  ${runs}`));
  ...
  if (indexCount === 0) {
    fail(
      `Index for strategy=${strategy} is empty. Index first:\n` +
  ...
  console.log(pc.bold("\nPhase 1: VRAM snapshot before warmup"));
  const vramBefore = captureVramSnapshot();
  console.log(pc.dim(`  GPU: ${vramBefore.gpuMemoryUsedMb}/${vramBefore.gpuMemoryTotalMb} MB, ollama procs: ${vramBefore.ollamaProcesses.length}`));

  console.log(pc.bold("\nPhase 2: warmup"));
  const warmStart = Date.now();
  await warmup(localLlmClient, localModel);
  console.log(pc.dim(`  warmup done in ${formatLatency(Date.now() - warmStart)}`));

  const vramAfterWarmup = captureVramSnapshot();
  console.log(pc.dim(`  GPU after warmup: ${vramAfterWarmup.gpuMemoryUsedMb}/${vramAfterWarmup.gpuMemoryTotalMb} MB`));
  if (vramAfterWarmup.partialOffload) {
    console.warn(pc.yellow(`  ⚠ partial CPU/GPU offload detected: ${JSON.stringify(vramAfterWarmup.ollamaProcesses)}`));
  }
  ...
  const modes = buildModes(reranker, queryRewriter, strategy, threshold, config.promptVariant);

  const ollamaOptions =
    config.numCtx !== undefined
      ? {
          numCtx: config.numCtx,
          numPredict: config.maxCompletionTokens,
  ...
  const refusedCount = Object.values(refusalResults).filter((r) => r?.refused === true).length;
  ...
  const judged = await dualJudge.judgeAll(results, questions as ControlQuestion[], {
    ...
    onJudgeDone: (j) => {
      const cPart = j.cloudJudge ? `C=${j.cloudJudge.score}` : pc.red("C=ERR");
      const lPart = j.localJudge ? `L=${j.localJudge.score}` : pc.red("L=ERR");
      process.stdout.write(pc.dim(`${cPart} ${lPart}\n`));
    },
  });
  ...
  const outputDir = "scripts/day29/raw";
...
main().catch((error) => {
  console.error(pc.red(`Day 29 eval failed`));
